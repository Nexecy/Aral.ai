...
/**
 * Trigger native in-app Google prompt.
 */
  ...
  if (initialized && window.google?.accounts?.id) {
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed()) {
        console.info('Google One Tap suppressed reason:', notification.getNotDisplayedReason());
      }
      if (notification.isSkippedMoment()) {
        console.info('Google One Tap skipped reason:', notification.getSkippedReason());
      }
    });
  }
}
